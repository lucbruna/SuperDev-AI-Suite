...
class MariaDBDriver(BaseDriver):
    """MariaDB driver — stdlib has no MariaDB client.

    MariaDB is wire-compatible with MySQL. Reuses MySQLDriver pattern.
    For production use, install aiomysql or mariadb connector.
    """

    async def connect(self, config: ConnectionConfig) -> None:
        self._config = config
        self._connected = True
        self._logger.info(f"MariaDB configured at {config.host}:{config.port or 3306}")
