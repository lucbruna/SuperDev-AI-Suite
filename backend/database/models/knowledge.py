...
from sqlalchemy import String, Text, Enum as SAEnum, ForeignKey, Integer, Boolean
...
class KnowledgeBase(Base, TimestampMixin):
    ...
    entries: Mapped[List["KnowledgeEntry"]] = relationship("KnowledgeEntry", back_populates="knowledge_base", cascade="all, delete-orphan")
...
class KnowledgeEntry(Base, TimestampMixin):
    ...
    id: Mapped[str] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, server_default=sa.text('uuid_generate_v4()'))
    knowledge_base_id: Mapped[str] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("knowledge_bases.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    chunks: Mapped[List["KnowledgeChunk"]] = relationship("KnowledgeChunk", back_populates="entry", cascade="all, delete-orphan")
...
class KnowledgeChunk(Base):
    ...
    id: Mapped[str] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, server_default=sa.text('uuid_generate_v4()'))
    entry_id: Mapped[str] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("knowledge_entries.id", ondelete="CASCADE"), nullable=False, index=True)
