...
class GroqProvider(BaseLLMProvider):
    ...
    def __init__(self, model: str = "llama3-70b", api_key: str = "") -> None:
        super().__init__(name="groq", model=model)
        self._api_key = api_key

    async def generate(self, prompt: str, **kwargs: Any) -> dict[str, Any]:
        return {
            "content": f"Groq ({self._model}) response to: {prompt[:50]}...",
            "success": True,
    ...
    async def generate_stream(self, prompt: str, **kwargs: Any) -> AsyncIterator[dict[str, Any]]:
        async def _gen() -> AsyncIterator[dict[str, Any]]:
            yield {"content": f"Groq ({self._model}) fast streaming...", "finish_reason": "stop"}
        return _gen()
