...
class NetworkMonitor:
    ...
    def get_active_connections(self) -> list[dict[str, Any]]:
        return [
            {"id": c.conn_id, "src": c.source_ip, "dst": c.dest_ip, "port": c.dest_port, "status": c.status.value}
    ...
    def get_suspicious_connections(self) -> list[dict[str, Any]]:
        return [
            {"id": c.conn_id, "src": c.source_ip, "dst": c.dest_ip, "port": c.dest_port}
            for c in self._connections.values()
            if c.status == ConnectionStatus.SUSPICIOUS
        ]
