...
class ThreatIndicator:
    def __init__(self, indicator_type: str, value: str, confidence: float = 0.5, source: IntelSource = IntelSource.INTERNAL) -> None:
        ...
        for ind in self._indicators.values():
            if ind.type == indicator_type or not indicator_type:
                if ind.confidence >= min_confidence:
                    results.append({"id": ind.indicator_id, "type": ind.type, "value": ind.value, "confidence": ind.confidence, "source": ind.source.value})
        return results
