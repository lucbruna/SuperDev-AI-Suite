    ...
    def check_traffic(self, source_ip: str, dest_ip: str, dest_port: int) -> dict[str, Any]:
        ...
        for rule in sorted(self._rules.values(), key=lambda r: r.priority):
            if not rule.enabled:
                continue
            if (
                self._match(rule.source, source_ip)
                and self._match(rule.destination, dest_ip)
                and (rule.port == 0 or rule.port == dest_port)
            ):
                ...
                return {
                    "allowed": rule.action == FirewallAction.ALLOW,
                    "action": rule.action.value,
                    "rule": rule.rule_id,
                }
