...
class EventSimulator:
    ...
    def schedule(self, event_type: str, time_step: int, data: Dict[str, Any] = None, priority: int = 0) -> Dict[str, Any]:
        event = {"type": event_type, "time_step": time_step, "data": data or {}, "priority": priority, "status": "scheduled"}
    ...
    def cancel(self, event_type: str = "", time_step: int = -1) -> int:
        ...
        if event_type and time_step >= 0:
            self._schedule = [e for e in self._schedule if not (e["type"] == event_type and e["time_step"] == time_step)]
        elif event_type:
