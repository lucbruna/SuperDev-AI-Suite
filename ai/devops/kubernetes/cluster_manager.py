...
class ClusterManager:
    ...
    def add_node(self, cluster_name: str, node_name: str, cpu: int = 4, memory_gb: int = 16) -> bool:
        if cluster_name not in self._clusters:
            return False
        self._clusters[cluster_name]["nodes"].append(
            {"name": node_name, "cpu": cpu, "memory_gb": memory_gb, "status": "ready"}
        )
        return True
    ...
    def remove_node(self, cluster_name: str, node_name: str) -> bool:
        if cluster_name not in self._clusters:
            return False
        self._clusters[cluster_name]["nodes"] = [
            n for n in self._clusters[cluster_name]["nodes"] if n["name"] != node_name
        ]
        return True
