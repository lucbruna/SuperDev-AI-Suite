...
print("\n[8] Alembic Migration:")
import os
migration_dir = "backend/database/migrations/versions"
migration_exists = os.path.exists("backend/database/migrations/env.py")
versions = [f for f in os.listdir(migration_dir) if f.endswith(".py") and f != "__init__.py"] if os.path.exists(migration_dir) else []
print(f"  env.py existe: {migration_exists}")
print(f"  Migration versions: {versions}")
assert migration_exists
assert len(versions) > 0, f"Nenhuma migration encontrada em {migration_dir}"
print(f"  Migration file: {versions[0]}")
print("  ✅ PASSOU")
