"""Persistent memory for the Self-Healing Engine.

Stores observations as keyed entries persisted atomically to JSON. Kept
deterministic and dependency-free, mirroring the Digital Twin memory store.
"""
...
class HealingMemoryError(RuntimeError):
    ...
    def save(self, path: str | Path) -> None:
        ...
        try:
            ...
            try:
                ...
                pass
            raise HealingMemoryError(f"failed to save memory: {exc}") from exc

    @classmethod
    def load(cls, path: str | Path) -> "HealingMemory":
        target = Path(path)
        ...
        try:
            ...
            raise HealingMemoryError(f"failed to load memory: {exc}") from exc
        memory = cls()
