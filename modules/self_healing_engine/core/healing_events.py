"""Event model and bus for the Self-Healing Engine.
...
EventHandler = Callable[["HealingEvent"], None]


@dataclass(slots=True)
class HealingEvent:
    """A single event produced by the healing runtime."""
...
class HealingEventBus:
    ...
    def publish(
        ...
        self._sequence += 1
        event = HealingEvent(
            type=event_type, payload=payload or {}, sequence=self._sequence
        )
    ...
    def history_of(self, event_type: str) -> list[HealingEvent]:
