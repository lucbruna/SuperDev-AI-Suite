"""Avatar logger — structured, bounded event log for the avatar engine."""
...
_LOGGER = logging.getLogger("ai_video_studio.avatar_engine")


class AvatarLogger:
    """Structured logger with a compact recent-events ring buffer."""
    ...
    def _emit(self, level: int, event: str, **context: Any) -> None:
        record = {"ts": round(time.time(), 3), "event": event,
                  "level": logging.getLevelName(level), **context}
...
def get_avatar_logger() -> AvatarLogger:
    """Return the shared avatar logger singleton."""
