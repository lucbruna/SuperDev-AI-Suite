"""Knowledge state — lifecycle state machine for the knowledge base.

Tracks the pipeline state (idle → scanning → parsing → building → indexing →
ready), records transitions and captures the last error with context.
"""
...
class KnowledgeState(str, Enum):
    """Lifecycle states of the knowledge base."""
    ...
    def terminal(self) -> bool:
        return self in {KnowledgeState.READY, KnowledgeState.STALE, KnowledgeState.ERROR}
...
class KnowledgeStateTracker:
    ...
    def __init__(self) -> None:
        self._state = KnowledgeState.UNINITIALIZED
    ...
    def set_state(self, new_state: KnowledgeState, context: str = "") -> None:
        ...
        if previous != new_state:
            ...
            if self._started_at is None and new_state not in {KnowledgeState.IDLE, KnowledgeState.UNINITIALIZED}:
                self._started_at = time.time()
            if new_state.terminal:
                self._finished_at = time.time()
            if new_state != KnowledgeState.ERROR:
                self._last_error = ""
    ...
    def mark_error(self, message: str, context: dict[str, Any] | None = None) -> None:
        ...
        self.set_state(KnowledgeState.ERROR, context="error")
